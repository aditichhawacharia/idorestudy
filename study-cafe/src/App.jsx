      ...
      >
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', marginBottom: '28px' }}>
          <div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
              <button onClick={() => setStudyVal(v => Math.max(1, (parseInt(v, 10) || 25) - 1))}
              ...
              <input type="number" min="1" max="120" value={studyVal} onChange={e => setStudyVal(e.target.value)}
              ...
              <button onClick={() => setStudyVal(v => Math.min(120, (parseInt(v, 10) || 25) + 1))}
                style={{ width: 36, height: 36, borderRadius: '10px', border: '2px solid #FFD7E5', background: 'white', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 900, fontSize: '18px', color: '#FF6B9D', flexShrink: 0 }}>+</button>
            </div>
          ...
          <div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
              <button onClick={() => setBreakVal(v => Math.max(1, (parseInt(v, 10) || 5) - 1))}
              ...
              <input type="number" min="1" max="60" value={breakVal} onChange={e => setBreakVal(e.target.value)}
              ...
              <button onClick={() => setBreakVal(v => Math.min(60, (parseInt(v, 10) || 5) + 1))}
                style={{ width: 36, height: 36, borderRadius: '10px', border: '2px solid #FFD7E5', background: 'white', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 900, fontSize: '18px', color: '#C86DD7', flexShrink: 0 }}>+</button>
            </div>
...
function AudioPlayer({ videoId, isMuted, isPaused }) {
  ...
  useEffect(() => {
    ...
    try {
      iframeRef.current.contentWindow.postMessage(JSON.stringify({ event: 'command', func: isPaused ? 'pauseVideo' : 'playVideo', args: [] }), '*');
    } catch (e) {}
  }, [isPaused, loaded]);
  ...
  return (
    <div style={{ position: 'fixed', bottom: '-600px', left: '-600px', width: '1px', height: '1px', opacity: 0, pointerEvents: 'none', zIndex: -1 }}>
      <iframe key={srcKey} ref={iframeRef} width="1" height="1"
        ...
        title="Background Music Player" frameBorder="0"
        allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share"
        referrerPolicy="strict-origin-when-cross-origin"
...
function TimerDoneNotif({ visible, wasStudying, onDismiss }) {
  if (!visible) return null;
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 99999, background: 'rgba(0,0,0,0.65)', backdropFilter: 'blur(10px)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <Confetti />
      <div className="timer-notif-in" style={{ position: 'fixed', top: '50%', left: '50%', background: 'linear-gradient(135deg, #FFF5F7 0%, #F5F3FF 50%, #FFF0F5 100%)', border: '5px solid #FFB6D9', borderRadius: '40px', padding: '56px 72px', boxShadow: '0 40px 120px rgba(255,107,157,0.55)', textAlign: 'center', maxWidth: '520px', width: '90vw', zIndex: 2 }}>
        <div style={{ fontSize: '96px', lineHeight: 1, marginBottom: '20px' }}>{wasStudying ? '🎉' : '📚'}</div>
        <h2 style={{ fontFamily: "'Playfair Display', serif", fontWeight: 900, fontSize: '38px', lineHeight: 1.15, background: 'linear-gradient(135deg, #FF6B9D, #C86DD7)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', marginBottom: '14px' }}>
        ...
        <button onClick={onDismiss} style={{ background: 'linear-gradient(135deg, #FF6B9D 0%, #C86DD7 100%)', color: 'white', border: 'none', borderRadius: '999px', padding: '18px 52px', fontSize: '19px', fontWeight: 900, fontFamily: "'Playfair Display', serif", cursor: 'pointer', boxShadow: '0 8px 32px rgba(255,107,157,0.45)' }}>
...
function TodoWidget() {
  ...
  return (
    <div style={{ position: 'fixed', bottom: '24px', right: '24px', zIndex: 8000, display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: '10px' }}>
      {isOpen && !isMinimized && (
        <div className="todo-slide-in" style={{ background: 'linear-gradient(135deg, rgba(255,245,247,0.99), rgba(245,243,255,0.99))', border: '3px solid #FFB6D9', borderRadius: '28px', padding: '20px 22px 22px', width: '340px', boxShadow: '0 24px 80px rgba(255,107,157,0.32)', backdropFilter: 'blur(20px)' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '14px' }}>
            ...
            <div style={{ display: 'flex', gap: '4px' }}>
              <button onClick={() => setIsMinimized(true)} style={{ background: 'none', border: 'none', cursor: 'pointer', padding: '4px', lineHeight: 0 }}><Minimize2 style={{ width: 16, height: 16, color: '#C86DD7' }} /></button>
              <button onClick={() => setIsOpen(false)} style={{ background: 'none', border: 'none', cursor: 'pointer', padding: '4px', lineHeight: 0 }}><X style={{ width: 16, height: 16, color: '#C86DD7' }} /></button>
            </div>
          </div>
          {total > 0 && (
          ...
          <div style={{ maxHeight: '220px', overflowY: 'auto', marginBottom: '12px', display: 'flex', flexDirection: 'column', gap: '7px' }}>
            ...
            {todos.map(todo => (
              <div key={todo.id} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '9px 11px', background: todo.done ? 'linear-gradient(135deg, #F0EBFF, #FFE5F1)' : 'white', borderRadius: '13px', border: `2px solid ${todo.done ? '#DDB6F5' : '#FFD7E5'}` }}>
                ...
                <button onClick={() => deleteTodo(todo.id)} style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 0, opacity: 0.38, flexShrink: 0, lineHeight: 0 }}
          ...
          <div style={{ display: 'flex', gap: '8px' }}>
            <input ref={inputRef} value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && addTodo()} placeholder="Add a task... ✨"
            ...
            <button onClick={addTodo} style={{ background: 'linear-gradient(135deg, #FF6B9D, #C86DD7)', border: 'none', borderRadius: '11px', padding: '9px 13px', cursor: 'pointer', display: 'flex', alignItems: 'center', boxShadow: '0 4px 12px rgba(255,107,157,0.35)' }}>
      ...
      {isOpen && isMinimized && (
        <div className="todo-slide-in" style={{ background: 'linear-gradient(135deg, rgba(255,245,247,0.99), rgba(245,243,255,0.99))', border: '3px solid #FFB6D9', borderRadius: '18px', padding: '10px 16px', boxShadow: '0 12px 40px rgba(255,107,157,0.25)', backdropFilter: 'blur(20px)', display: 'flex', alignItems: 'center', gap: '10px' }}>
          ...
          <button onClick={() => setIsMinimized(false)} style={{ background: 'none', border: 'none', cursor: 'pointer', padding: '2px', lineHeight: 0 }}><Maximize2 style={{ width: 15, height: 15, color: '#C86DD7' }} /></button>
          <button onClick={() => setIsOpen(false)} style={{ background: 'none', border: 'none', cursor: 'pointer', padding: '2px', lineHeight: 0 }}><X style={{ width: 15, height: 15, color: '#C86DD7' }} /></button>
        </div>
      )}
      <button onClick={() => { setIsOpen(o => !o); setIsMinimized(false); }}
    ...
    <footer style={{
      ...
      <div style={{ textAlign: 'center', marginTop: '16px' }}>
        <p style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: '12px', color: '#D4B8E0', fontStyle: 'italic' }}>
          © {new Date().getFullYear()} IdoréStudy · Made with 💖 for students everywhere · All embedded videos remain the property of their respective owners.
        </p>
      </div>
    </footer>
  );
...
const StudyCafe = () => {
  ...
  const studyBuddies = [
    { id: 1, name: 'Rosé', group: 'BLACKPINK', videoId: 'oadMhHMubQ4?si=fdyQ5nAQQgIZ2KC2', image: 'https://media.vogue.co.uk/photos/602503e953a6c957c223e1c2/1:1/w_1079,h_1080,c_limit/65422624_362861021078977_3034176141535337881_n.jpg' },
    { id: 2, name: 'Jennie', group: 'BLACKPINK', videoId: 'Fe8kR3W9VGA?si=lOhkuY6w6hc7vlRl', image: 'https://cdn-images.dzcdn.net/images/artist/56c65ac9ea451119ddc8c0b02915d103/1900x1900-000000-80-0-0.jpg' },
    { id: 3, name: 'Lisa', group: 'BLACKPINK', videoId: 'tX9rWUJUGbk?si=1cwwtiNJNLHZ6BkT', image: 'https://cdn.giaoducthoidai.vn/images/b4508baace0d9fe4c8bbd296e259642eeddd55e67f3e3a0e2128ffa2d903782b4e633d640eb7428ab5b72e0feb356db5a8904041fc31274acb4faf5c0a71469e/lisa-3709.jpg' },
    { id: 5, name: 'Jungkook', group: 'BTS', videoId: 'xy_mVVv4Oc0?si=XIcdkXpt9NFM70IL', image: 'https://media.allure.com/photos/5d51932d40395d0008565c27/1:1/w_1117,h_1117,c_limit/BTS%20Jungkook%20The%20Fact%20Music%20Awards.jpg' },
    { id: 6, name: 'V (Taehyung)', group: 'BTS', videoId: 'Xt2wCvkSegU?si=RQO_W8pmXh7KOzb-', image: 'https://img.wattpad.?s=fit&w=720&h=720' },
    { id: 7, name: 'Jimin', group: 'BTS', videoId: 'KxE4i8-nYEs?si=C5vcXDoBao4Ubwmr', image: 'https://i0.wp.com/zaloramalaysiablog.wpcomstaging.com/wp-content/uploads/2025/10/JIMIN-FEATURE.jpeg?resize=736%2C768&ssl=1' },
    { id: 12, name: 'Wonyoung', group: 'IVE', videoId: 'ZZaA0c-PsXc?si=EZJuOje8auDVqNQT', image: 'https://upload.wikimedia.org/wikipedia/commons/6/6f/Jang_Won-young_of_Ive%2C_April_16%2C_2025.png' },
    { id: 15, name: 'Chaewon', group: 'LE SSERAFIM', videoId: '3one8kjAAQI?si=Lx74Rk6W62Mk1YWv', image: 'https://cdn-images.dzcdn.net/images/artist/e6f96dce8a3460c7e8f4e1ae8fe6b6a3/1900x1900-000000-80-0-0.jpg' },
    { id: 17, name: 'Kazuha', group: 'LE SSERAFIM', videoId: 'KznmfOQrK_E?si=I-mvB2p7JTPKhA8m', image: 'https://image.koreaboo.com/2025/11/le-sserafim-kazuha-is-selected-as-the-new-model-for-french-v0-3i7ttzuwwxqc1-519x640.jpg' },
    { id: 18, name: 'Karina', group: 'aespa', videoId: 'r6OQFloCDZw?si=ajwYkdn7wDS7p1nR', image: 'https://i.pinimg.com/564x/76/b6/12/76b612ff257c2caadc1090e633646cbe.jpg' },
    { id: 22, name: 'Minji', group: 'NewJeans', videoId: 'LhIivrX4gKk?si=VX33cAITX0kpqKE_', image: 'https://i.redd.it/newjeans-minji-harpers-bazaar-korea-october-2023-issue-v0-0iwgev6ow3pb1.jpg?width=2048&format=pjpg&auto=webp&s=8473457a93c0c0cb7123d6766795089cdcfd957c' },
    { id: 24, name: 'Seulgi', group: 'Red Velvet', videoId: 'UZOnLTTiZKY?si=yeswGn2PlAAqNpgf', image: 'https://cdn.i-scmp.com/sites/default/files/styles/768x768/public/images/methode/2019/02/08/35c9b9ee-2b4c-11e9-8864-9e8ab15a22ca_image_hires_125930.JPG?itok=JnlideOe&v=1549601969' },
    { id: 27, name: 'Bang Chan', group: 'Stray Kids', videoId: 'ANuQjiEMMcU?si=85lRUokCKtNzHBJm', image: 'https://upload.wikimedia.org/wikipedia/commons/thumb/a/ab/190106_Stray_Kids_%E6%96%B9%E7%87%A6.png/250px-190106_Stray_Kids_%E6%96%B9%E7%87%A6.png' },
    { id: 28, name: 'Felix', group: 'Stray Kids', videoId: 'EYpwvrJlV-s?si=3BQrSp7AtwvOucAI', image: 'https://external-preview.redd.it/stray-kids-felix-becomes-the-new-face-of-gong-cha-v0-Y1dU4kACbSQRpJAnGX4dI9qrjrRZMif9kS9VM06lbSw.jpg?width=640&crop=smart&auto=webp&s=e28a6c99ae4995b2fca6fbdba49386fbebcabc1f' },
    { id: 29, name: 'Han', group: 'Stray Kids', videoId: 'TMjrez7sv5o?si=z553RJcHnTC7JXpf', image: 'https://cdn.kpopping.com/idols/Han/profile.webp' },
    { id: 30, name: 'Hyunjin', group: 'Stray Kids', videoId: 'QFfZlBdAhgs?si=dYxdJAq3oc4V6R4J', image: 'https://upload.wikimedia.org/wikipedia/commons/0/0e/Hyunjin_of_Stray_Kids%2C_September_24%2C_2025.png' },
    { id: 31, name: 'Lee Know', group: 'Stray Kids', videoId: '9CKliRdrawg?si=9G33xVOldR2a3nIa', image: 'https://www.kpoppinkstore.com/cdn/shop/files/ALEEKNOW_620x.webp?v=1739838906' },
    { id: 32, name: 'Yunah', group: 'ILLIT', videoId: 'Kz5ie0SAPJM?si=VfoZlZkZ1t2Blwoc', image: 'https://www.billboard.com/wp-content/uploads/2024/06/ILLIT-Rookie-Spotlight-YUNAH-billboard-1240.jpg?w=800' },
    { id: 33, name: 'Wonhee', group: 'ILLIT', videoId: 'gY5nbjT8ZYU?si=jWYNzxoQb0eYuhmb', image: 'https://yt3.googleusercontent.com/XKcAXSDCdTjZbK1L-kXT0v61K-tw6xwzPn9aMmPUdbmMW8mMygmkJswoXdlMJU7DNm_oifQ8mw=s900-c-k-c0x00ffffff-no-rj' },
    { id: 34, name: 'Moka', group: 'ILLIT', videoId: 'fwMMBtUucng?si=kLOwcp6PNzugbUhe', image: 'https://preview.redd.it/are-there-any-updates-on-moka-v0-tpht1eanzowf1.jpeg?auto=webp&s=e9cb1613078635750d1b66c83370ef38a0adadfc' },
    { id: 35, name: 'Chuu', group: 'LOONA', videoId: 'bDQRKF4jTuQ?si=YZe4cd0s_7EZShDc', image: 'https://i.pinimg.com/236x/61/ca/c0/61cac0fbfccbc41b5137cb87bb03fd00.jpg' },
    { id: 36, name: 'Yuna', group: 'ITZY', videoId: 'iLzKAgu_5g4?si=9mjs1w33ymMcjfS_', image: 'https://pbs.twimg.com/media/E6OrSSsWYAE-Naw.jpg' },
    { id: 37, name: 'Key', group: 'SHINEE', videoId: 'lMqr_YXI9IM?si=jH8UmTxaTCakVUiP', image: 'https://nolae.eu/cdn/shop/articles/key-shinee-profil-731664.jpg?v=1723729868&width=1200' },
    { id: 38, name: 'Dahyun', group: 'TWICE', videoId: '47ocn-7vw-E?si=Xkf8ccNzCBwcJ1p4', image: 'https://i.pinimg.com/736x/44/1f/22/441f225db0f809e10a0b9abfda93cca5.jpg' },
    { id: 39, name: 'Yuqi', group: 'G-IDLE', videoId: 'gKIGXBkW56Y?si=vP0xmCSwLFDJD9Fq', image: 'https://external-preview.redd.it/231016-official-update-yuqi-on-star-of-star-girls-v0-cPdL1Wl6mbHAWjHnatUUhlDUCVUsmVTibuqTseTjQxQ.jpg?auto=webp&s=53fa9f7fd7c5b2ad148b1d0137720030f5a2c8bf' },
  ...
  return (
    <div className="min-h-screen flex relative overflow-hidden" style={{ backgroundColor: '#2D2D3D' }}>
      ...
      <div className="flex-1 relative h-screen overflow-hidden">
        ...
        {selectedMusic && selectedMusic.videoId && <AudioPlayer videoId={selectedMusic.videoId} isMuted={isMuted} isPaused={isMusicPaused} />}
        ...
        <div ref={timerRef} className="absolute z-30 cursor-move"
          style={{ top: timerPosition.y === 0 ? '24px' : `${timerPosition.y}px`, right: timerPosition.x === 0 ? '24px' : 'auto', left: timerPosition.x !== 0 ? `${timerPosition.x}px` : 'auto' }}
          ...
          {isTimerMinimized ? (
            <div className="rounded-2xl shadow-2xl p-4 border-4 border-pink-200 backdrop-blur-md"
              ...
              <div className="flex items-center gap-3">
                ...
                <button onClick={(e) => { e.stopPropagation(); setIsRunning(r => !r); }} className="p-2 rounded-xl bg-pink-100 hover:bg-pink-200">
                  {isRunning ? <Pause className="w-4 h-4 text-pink-600" /> : <Play className="w-4 h-4 text-pink-600" />}
                </button>
                <button onClick={(e) => { e.stopPropagation(); setIsTimerMinimized(false); }} className="p-2 rounded-xl bg-purple-100 hover:bg-purple-200">
            ...
            <div className="rounded-3xl shadow-2xl p-5 w-72 border-4 border-pink-200" style={{ background: 'linear-gradient(135deg, #FFF5F7 0%, #F5F3FF 100%)' }}>
              <div className="flex items-center justify-between mb-3">
                ...
                <div className="flex gap-1">
                  <button onClick={(e) => { e.stopPropagation(); setIsTimerMinimized(true); }} className="p-1.5 hover:bg-pink-100 rounded-xl transition">
                    <Minimize2 className="w-3.5 h-3.5 text-gray-600" />
                  </button>
                  <button onClick={(e) => { e.stopPropagation(); setShowTimerSettings(true); }} className="p-1.5 hover:bg-pink-100 rounded-xl transition">
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', marginBottom: '10px' }}>
                <span style={{ fontFamily: "'Cormorant Garamond', serif", fontStyle: 'italic', fontSize: '11px', color: '#B8A0CC', display: 'flex', alignItems: 'center', gap: '4px' }}>
                  ...
                  <button onClick={(e) => { e.stopPropagation(); setShowTimerSettings(true); }}
        ...
        <div className="absolute bottom-6 left-1/2 transform -translate-x-1/2 z-20">
          ...
          {isMenuBarMinimized ? (
            <div className="menu-bar-slide-in flex items-center gap-3 px-5 py-3 rounded-full shadow-2xl backdrop-blur-xl border-3"
              style={{ background: 'linear-gradient(135deg, rgba(255,107,157,0.95), rgba(200,109,215,0.95))', borderColor: 'rgba(255,255,255,0.3)' }}>
              ...
              <button onClick={() => setIsMenuBarMinimized(false)} className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-white/20 hover:bg-white/35">
                <ChevronUp className="w-4 h-4 text-white" />
                <span className="text-white text-xs font-bold">Show</span>
              </button>
            </div>
          ) : (
            <div className="flex items-center gap-4 px-9 py-5 rounded-full shadow-2xl backdrop-blur-xl border-3"
              style={{ background: 'linear-gradient(135deg, rgba(255,107,157,0.95), rgba(200,109,215,0.95))', borderColor: 'rgba(255,255,255,0.3)' }}>
              <button onClick={() => setIsMuted(m => !m)} className="p-3 rounded-full bg-white/20 hover:bg-white/30 transition transform hover:scale-110">
                {isMuted ? <VolumeX className="w-6 h-6 text-white" /> : <Volume2 className="w-6 h-6 text-white" />}
              </button>
              <button onClick={() => setIsMusicPaused(p => !p)} className="p-3 rounded-full bg-white/20 hover:bg-white/30 transition transform hover:scale-110">
                {isMusicPaused ? <Play className="w-6 h-6 text-white" fill="white" /> : <Pause className="w-6 h-6 text-white" fill="white" />}
              </button>
              <button onClick={() => { setShowMusicMenu(s => !s); setShowCafeMenu(false); }}
              ...
              <button onClick={() => { setShowCafeMenu(s => !s); setShowMusicMenu(false); }}
              ...
              <button onClick={() => { setIsMenuBarMinimized(true); setShowMusicMenu(false); setShowCafeMenu(false); }}
...
// ── Layout wrapper: shows Navbar on non-study pages ───────────────────────────
function Layout({ children }) {
  const location = useLocation();
  const isStudyRoot = location.pathname === '/';
  // On the study page, we don't show the navbar (it's a full-screen immersive experience)
  return (
    <>
      {!isStudyRoot && <Navbar />}
      {children}
